...
# "total_questions":len(question),
    ...
    def get_questions_by_search_term():
        ...
        questions = Question.query.filter(
                                         Question.question.ilike(look_for)
                                         ).all()
        if not questions:
            abort(404)
        else:
    ...
    def get_question_by_category(categor_id):
        questions = Question.query.filter(
                                         Question.category == categor_id
                                         ).all()
        category = Category.query.filter(
                                        Category.id == categor_id
                                        ).one_or_none()
        if not questions:
            abort(404)
        else:
    ...
    def quizzes():
        ...
        if quiz_category['id'] == 0:
            ...
            qusetions = Question.query.filter(Question.category
                                              == quiz_category['id']).all()
            category = Category.query.filter(Category.id
                                             == quiz_category['id']).all()
        if not qusetions:
    ...
    def error_handler_404(error):
        return jsonify({"success": False, 'error': 404,
                        "message":
                        "bad request, the resouse could not be found"})

    @app.errorhandler(422)
    def error_handerl_422(error):
        return jsonify({"succes": False, 'error': 422,
